...
labels = torch.load('speechcommands_labels.pth')
index_to_labels = {ind: lab for ind, lab in enumerate(labels)}
model = AudioClassifier(num_classes=len(labels))
model.load_state_dict(torch.load('speech_commands_model.pth', map_location=device))
model.to(device)
model.eval()
...
def change_audio(waveform, sample_rate):
    if sample_rate != 16000:
        new_sr = transform.Resample(orig_freq=sample_rate, new_freq=16000)
